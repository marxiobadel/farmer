...
import type { BreadcrumbItem, PaginationMeta, Country, Zone } from '@/types';
...
export default function Index({ zones, countries, filters }: PageProps) {
    ...
    const toggleSort = (column: keyof Zone) => {
    ...
    const applyFilters = (newFilters: Partial<PageProps["filters"]> & { page?: number }) => {
        router.get(admin.zones.index().url, {
            search,
            sort,
            per_page: perPage,
            page: zones.meta.current_page,
            ...newFilters,
        }, { preserveState: true, replace: true });
    }

    const handleEdit = (zone: Zone) => setEditingZone(zone);
        ...
        <AppLayout breadcrumbs={isMobile ? [] : breadcrumbs}>
            ...
            <CarriersLayout>
                <div className="space-y-6 md:mt-[-77px]">
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Gérez les zones de livraison.</p>
                        </div>
                        <div className="flex gap-2 w-full md:w-auto">
                            <div className='relative'>
                                <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                <Input
                                    placeholder={isMobile ? "Rechercher..." : "Rechercher une zone..."}
                                    value={search}
                ...
                <ZoneForm
                    ...
                    zone={editingZone}
                    submitUrl={
                        editingZone ? admin.zones.update(editingZone.id).url : admin.zones.store().url
                    }
                ...
                <ConfirmDeleteDialog
                    ...
                    message={deleteZone
                        ? `Voulez-vous vraiment supprimer ${deleteZone.name} ? Cette action est irréversible.`
                        : `Voulez-vous vraiment supprimer ${Object.keys(rowSelection).length} catégorie(s) ? Cette action est irréversible.`}
                    onConfirm={() => {
                        const ids = deleteZone ? [deleteZone.id]
                                    ...
                                    toast.success(
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-sm text-muted-foreground">
                                                Les zones sélectionnées ont été supprimées.
                                            </span>
                                        </div>
                                    );
                                },
